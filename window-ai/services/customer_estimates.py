...
def _customer_door_openings(
    ...
    for project_opening, opening in zip(project_openings, quote_openings):
        ...
        remaining = sell
        for index, (base, description, qty) in enumerate(zip(bases, descriptions, quantities)):
        ...
        label = str(project_opening.get("description") or "").strip() or str(opening.get("label") or "Door opening")
        customer_openings.append(
            {
                "id": project_opening.get("id"),
...
def price_customer_estimate(
    *,
    ...
    if doors:
        ...
        door_openings = _customer_door_openings(doors, door_quote.get("openings", []))
